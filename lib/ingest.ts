...
type JsonType = 'circulaires' | 'notes' | 'faq' | 'tarifs' | 'procedures' | 'glossaire' | 'decisions'
...
async function ingestJSON(file: FormFile): Promise<IngestResult> {
  ...
  try {
    ...
    // ── Format schéma v2 (projet Claude dédié) ──────────────────────────────
    // Un seul document circulaire par fichier : { circulaire: {...}, chunks: [...] }.
    // Pas de wrapper "type"/"entries" — on le détecte à la forme, avant le
    // dispatch normal, et on route vers l'ingestion qui préserve les métadonnées
    // (statut, domaine, article, abroge_ou_modifie...) au lieu de re-chunker.
    if (raw && typeof raw === 'object' && 'circulaire' in raw && 'chunks' in raw) {
      const result = await ingestCirculaireSchemaV2(raw, 'projet-claude-json')
      const statusMap: Record<typeof result.status, IngestResult['status']> = {
        inserted:           'success',
        skipped_doublon:    'skipped',
        rejected_invalide:  'error',
        error:              'error',
      }
      return {
        fichier,
        ...
        entriesInserted: result.status === 'inserted' ? 1 : undefined,
        error:           result.errors?.join('; '),
      }
    }

    // ── Format schéma v2 — notes ADII ──────────────────────────────────────
    // Même détection à la forme, mais clé racine "note" : route vers
    // notes_adii / notes_adii_chunks, jamais vers circulaires.
    if (raw && typeof raw === 'object' && 'note' in raw && 'chunks' in raw) {
      const result = await ingestNoteSchemaV2(raw, 'projet-claude-json')
      const statusMap: Record<typeof result.status, IngestResult['status']> = {
        inserted:           'success',
        skipped_doublon:    'skipped',
        rejected_invalide:  'error',
    ...
    if (!type || !Array.isArray(entries) || entries.length === 0)
      throw new Error('Format invalide — champs "type" et "entries" requis (ou format schéma v2 { circulaire, chunks })')
